...
static void usage(u8 *argv0, int more_help) {
  ...
  SAYF(
      ...
      "DeepFuzz settings (config co-execution + depth model):\n"
      "  -r file       - JSON grammar file describing command-line options\n"
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while ((opt = getopt(
    ...
    switch (opt) {
      ...
      case 'J':                                                 /* depth model */
        ...
        if (afl->depth_model_path) { FATAL("Multiple -J options not supported"); }
        afl->depth_model_path = ck_strdup(optarg);
        break;

      case 'r':                                               /* grammar file */

        if (afl->grammar_file) { FATAL("Multiple -r options not supported"); }
        afl->grammar_file = ck_strdup(optarg);
        break;
  ...
  /* ===== DeepFuzz Initialization ===== */
  if (afl->grammar_file || afl->depth_model_path) {

    /* Init config co-execution framework */
    if (afl->grammar_file) {
      ...
      /* generate initial configs (either from file or via argv_havoc) */
      if (afl->size_of_inject_arr == 0) {

        generate_initial_configs(afl);

      }

      OKF("DeepFuzz config co-exec initialized with %d configs",
          afl->size_of_inject_arr);

    }

    /* Load depth model */
    if (afl->depth_model_path) {

      deepfuzz_load_depth_model(afl, afl->depth_model_path);
      afl->depth_mode_active = 1;

      /* Init depth shared memory */
      afl->depth_shm_id = shmget(IPC_PRIVATE, DEEPFUZZ_DEPTH_SHM_SIZE,
                                 IPC_CREAT | IPC_EXCL | 0600);
      if (afl->depth_shm_id < 0) {

        WARNF("Could not create depth SHM, depth mode disabled");
        afl->depth_mode_active = 0;

      } else {

        afl->depth_shm = (depth_shm_data_t *)shmat(afl->depth_shm_id, NULL, 0);
        if (afl->depth_shm == (void *)-1) {

          WARNF("Could not attach depth SHM, depth mode disabled");
          afl->depth_mode_active = 0;
          afl->depth_shm = NULL;
          ...
          /* pass SHM ID to child processes via env */
          char shm_id_str[32];
          snprintf(shm_id_str, sizeof(shm_id_str), "%d", afl->depth_shm_id);
          setenv("__DEEPFUZZ_DEPTH_SHM_ID", shm_id_str, 1);

          OKF("Depth model loaded, SHM initialized");
    ...
    if (!afl->execs_per_config) { afl->execs_per_config = 5000; }
    afl->execs_on_config = 0;
    afl->current_config_idx = 0;

    /* Apply first config to forkserver */
    if (afl->configs_to_inject_str && afl->size_of_inject_arr > 0) {

      inject_config_str_to_fsrv(afl, afl->configs_to_inject_str[0]);

    }

  }
  /* ===== End DeepFuzz Initialization ===== */
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      /* ===== DeepFuzz: Config Switching & Periodic Reload ===== */

      if (afl->size_of_inject_arr > 1 && afl->configs_to_inject_str) {

        afl->execs_on_config++;

        /* Switch to next config every execs_per_config executions */
        if (afl->execs_on_config >= afl->execs_per_config) {

          afl->current_config_idx =
              (afl->current_config_idx + 1) % afl->size_of_inject_arr;
          inject_config_str_to_fsrv(
        ...
        /* Periodic config reload from Python layer (every 60s) */
        time_t now = time(NULL);
        if (now - afl->last_config_reload >= 60) {

          afl->last_config_reload = now;

          /* Check for updated config_set.json */
          if (afl->out_dir) {

            char cfg_path[PATH_MAX];
            snprintf(cfg_path, PATH_MAX, "%s/config_set.json", afl->out_dir);
            if (access(cfg_path, R_OK) == 0) {

              /* reload config pool from file */
              /* (simplified: just read line-by-line for now) */
              FILE *fc = fopen(cfg_path, "r");
              if (fc) {

                /* free old configs */
                for (u32 i = 0; i < afl->config_pool_size; i++) {

                  ck_free(afl->config_pool[i]);

                }

                ck_free(afl->config_pool);
                afl->config_pool = NULL;
                afl->config_pool_size = 0;
                ...
                while (getline(&line, &llen, fc) != -1) { n++; }
                rewind(fc);

                afl->config_pool = ck_alloc(sizeof(u8 *) * n);
                u32 idx = 0;

                while (getline(&line, &llen, fc) != -1 && idx < n) {

                  if (line[0] != '\n' && line[0] != '\0') {

                    size_t ln = strlen(line);
                    if (ln > 0 && line[ln - 1] == '\n') { line[ln - 1] = '\0'; }
                    afl->config_pool[idx++] = strdup(line);

                  }

                }

                afl->config_pool_size = idx;
                free(line);
                fclose(fc);
...
stop_fuzzing:
  ...
  /* Detach and delete depth shared memory */
  if (afl->depth_shm && afl->depth_shm != (void *)-1) {
  ...
  if (afl->depth_shm_id >= 0) {
    ...
    shmctl(afl->depth_shm_id, IPC_RMID, NULL);
  ...
  /* Free config injection arrays */
  if (afl->configs_to_inject_str) {

    for (int i = 0; i < afl->size_of_inject_arr; i++) {

      ck_free(afl->configs_to_inject_str[i]);

    }

    ck_free(afl->configs_to_inject_str);

  }

  if (afl->havoc_argv_scores) { ck_free(afl->havoc_argv_scores); }
  if (afl->crashed_arr) { ck_free(afl->crashed_arr); }
  ...
  /* Free config local bitmaps */
  if (afl->configs_to_inject_local_bitmap) {

    for (int i = 0; i < afl->size_of_inject_arr; i++) {

      ck_free(afl->configs_to_inject_local_bitmap[i]);

    }

    ck_free(afl->configs_to_inject_local_bitmap);
  ...
  if (afl->options_holder) {
    ...
    for (u32 i = 0; i < afl->options_count; i++) {

      ck_free(afl->options_holder[i]);
      ck_free(afl->options_type[i]);
      ck_free(afl->options_connectors[i]);
      ck_free(afl->options_ranges[i]);

    }

    ck_free(afl->options_holder);
    ck_free(afl->options_type);
    ck_free(afl->options_ranges);
    ck_free(afl->options_choices);
    ck_free(afl->options_connectors);
    ck_free(afl->options_choices_count);

  }

  /* Free depth model entries */
  if (afl->depth_model.entries) { ck_free(afl->depth_model.entries); }
