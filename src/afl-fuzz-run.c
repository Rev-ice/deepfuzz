...
u8 __attribute__((hot)) common_fuzz_stuff(afl_state_t *afl, u8 *out_buf,
  ...
  /* ===== DeepFuzz Config Co-execution Loop ===== */
  if (afl->size_of_inject_arr > 1 && afl->configs_to_inject_str) {

    u8  *virgin_holder = ck_alloc(afl->fsrv.map_size);
    u8  *fsrv_trace_max = ck_alloc(afl->fsrv.map_size);
    bool new_bits_found = false;
    bool crashed = false;
    u32  max_new_bits = 0;
    u32  max_new_bits_config = 0;

    if (afl->crashed_arr) { ck_free(afl->crashed_arr); }
    afl->crashed_arr = ck_alloc(afl->size_of_inject_arr * sizeof(int));

    for (int config_counter = 0;
         config_counter < afl->size_of_inject_arr;
         config_counter++) {

      u32 cur_new_bits = 0;
      afl->crashed_arr[config_counter] = -1;

      memcpy(virgin_holder, afl->virgin_bits, afl->fsrv.map_size);

      inject_config_str_to_fsrv(afl,
                                afl->configs_to_inject_str[config_counter]);

      if (afl->configs_to_inject_local_bitmap[config_counter] == NULL) {

        afl->configs_to_inject_local_bitmap[config_counter] =
            ck_alloc(afl->fsrv.map_size);
      ...
      if (afl->stop_soon) {
        ...
        ck_free(virgin_holder);
        ck_free(fsrv_trace_max);
        return 1;

      }

      cur_new_bits = has_new_bits(afl, virgin_holder);
      ...
      /* Record data_triple for feedback */
      if (afl->data_triples) {

        afl->data_triples[config_counter][afl->execs_counter].new_bits =
            (u8)(cur_new_bits > 0 ? 1 : 0);
        afl->data_triples[config_counter][afl->execs_counter].crashed =
            (fault == FSRV_RUN_CRASH) ? 1 : 0;
        afl->data_triples[config_counter][afl->execs_counter].id =
            (u32)afl->queued_items;
      ...
      /* Accumulate per-config trace bits */
      for (u32 i = 0; i < afl->fsrv.map_size; i++) {
        ...
        afl->configs_to_inject_local_bitmap[config_counter][i] |=
            afl->fsrv.trace_bits[i];
    ...
    /* Re-inject the best config for subsequent executions */
    if (new_bits_found) {

      inject_config_str_to_fsrv(afl,
          afl->configs_to_inject_str[max_new_bits_config]);

    }

    if (crashed) { fault = FSRV_RUN_CRASH; }
    ...
    /* Standard single-execution path (no config co-exec) */
    fault = fuzz_run_target(afl, &afl->fsrv, afl->fsrv.exec_tmout);

  }
  /* ===== End DeepFuzz Config Co-execution Loop ===== */
  ...
  /* DeepFuzz: compute seed depth after execution */
  if (afl->depth_mode_active && afl->queue_cur) {

    deepfuzz_compute_seed_depth(afl, afl->queue_cur);
