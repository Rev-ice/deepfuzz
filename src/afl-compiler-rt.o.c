...
/* ===== DeepFuzz: Runtime Call Depth Instrumentation =====
 *
 * Uses -finstrument-functions to insert __cyg_profile_func_enter/exit
 * at every function call boundary. Depth data is stored in an 8-byte
 * POSIX shared memory segment identified by __DEEPFUZZ_DEPTH_SHM_ID.
 *
 * Compile target with:
 *   CFLAGS="-finstrument-functions -fno-optimize-sibling-calls"
 *
...
/* must match depth_shm_data_t in deepfuzz-depth.h */
struct __deepfuzz_depth_shm {

  unsigned int max_call_depth;
  unsigned int cur_call_depth;

};

static struct __deepfuzz_depth_shm *__afl_depth_shm = NULL;

/* thread-local depth counters */
__thread unsigned int __afl_depth_current = 0;
__thread unsigned int __afl_depth_max = 0;

__attribute__((constructor(102))) static void __deepfuzz_depth_init(void) {

  const char *shm_id_str = getenv("__DEEPFUZZ_DEPTH_SHM_ID");
  ...
  if (shm_id_str) {
    ...
    if (shm_id >= 0) {
      ...
      __afl_depth_shm = (struct __deepfuzz_depth_shm *)shmat(shm_id, NULL, 0);

      if (__afl_depth_shm == (void *)-1) {
...
void __cyg_profile_func_enter(void *func, void *caller) {
  ...
  if (__afl_depth_current > __afl_depth_max) {

    __afl_depth_max = __afl_depth_current;
  ...
  if (__afl_depth_shm) {
    ...
    __afl_depth_shm->cur_call_depth = __afl_depth_current;

    if (__afl_depth_current > __afl_depth_shm->max_call_depth) {

      __afl_depth_shm->max_call_depth = __afl_depth_current;
