    ...
    for (i = 0; i < n; i++) {
      ...
      if (likely(!q->disabled)) {
        ...
        {  // inline does result in a compile error with LTO, weird
          ...
          /* DeepFuzz: depth factor for weight-based selection */
          if (afl->depth_mode_active && afl->depth_model.loaded &&
              afl->depth_model.max_depth > 0.0) {

            double d = q->depth_info.max_combined_depth;
            if (d > 0.0) {

              weight *= 1.0 + 0.5 * (d / afl->depth_model.max_depth);
...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det) {
  ...
  /* DeepFuzz: init depth info */
  memset(&q->depth_info, 0, sizeof(seed_depth_info_t));
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  /* DeepFuzz: depth-based energy bonus */
  if (afl->depth_mode_active && afl->depth_model.loaded) {

    double bonus = deepfuzz_depth_bonus(afl, q);
    if (bonus > 0.0) {

      perf_score = (u32)((double)perf_score * (1.0 + bonus));
      if (perf_score > afl->havoc_max_mult * 100) {
