...
/* --- DeepFuzz / VAFuzz Data Types --- */

struct data_triple {

  u32 id;        /* entry number in queue_buf          */
  u8  crashed;    /* Did the testcase crash?            */
  u8  new_bits;
...
struct queue_entry {
  ...
  /* DeepFuzz: per-seed depth statistics */
  seed_depth_info_t depth_info;
...
#define FOREIGN_SYNCS_MAX 32U
  ...
  /* ===== DeepFuzz: Config Co-execution State ===== */

  u8   **configs_to_inject_str;      /* array of config strings to inject      */
  int    size_of_inject_arr;         /* count of configs in the array          */
  u128  *energy_ls;                  /* energy values per config               */
  u128  *havoc_argv_scores;          /* scores for config-level mutation       */
  u128   execs_counter;              /* execution counter for data_triples     */
  u128   execs_per_selection;        /* execs before needing new configs       */
  u32    config_count;               /* total number of possible configs       */
  u8     havoc_argv;                 /* flag: use argv-level mutation          */
  struct data_triple **data_triples; /* per-config feedback triples            */
  struct HashOptMap  *options_hashmap;/* option lookup from grammar             */
  u8   **options_holder;             /* array of option strings                */
  u8   **options_type;               /* array of option types                  */
  int  **options_ranges;             /* option range [min, max]                */
  u8  ***options_choices;            /* option choice arrays                   */
  int   *options_choices_count;      /* count per choice array                 */
  u8   **options_connectors;         /* option connectors                      */
  u128   options_count;              /* number of options                      */
  char  *temp_config_file_path;      /* path to temporary config file          */
  u8   **configs_to_inject_local_bitmap;  /* per-config local bitmaps          */
  int   *crashed_arr;                /* per-config crash record                */
  u128   execs_per_config;           /* execs between config switches          */
  u128   execs_on_config;            /* execs done on current config            */
  u32    current_config_idx;         /* current config index                   */
  u8    *configuration_file;         /* initial config file path               */
  u8    *grammar_file;               /* JSON grammar file path                 */

  /* ===== DeepFuzz: Depth Model State ===== */

  depth_shm_data_t *depth_shm;       /* shared memory for runtime depth        */
  s32               depth_shm_id;    /* POSIX shm id for depth SHM             */
  depth_model_t     depth_model;     /* loaded depth model data                */
  u8               *depth_model_path;/* path to depth_model.json               */
  u8                depth_mode_active;/* flag to toggle depth mode              */

  /* ===== DeepFuzz: Affinity Log ===== */
  ...
  u8  **config_pool;                 /* config strings loaded from JSON         */
  u32   config_pool_size;            /* number of configs in pool              */
  time_t last_config_reload;         /* timestamp of last config reload        */
